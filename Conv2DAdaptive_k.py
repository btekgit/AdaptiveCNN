...
def test():
   ...
   # model.summary()
    ...
    multipliers = {'acnn/Sigma:0': 0.1, 'acnn/Weights:0':(0.5)}
    opt = LearningRateMultiplier(SGD, lr_multipliers=multipliers, 
                             lr=1.0, momentum=0.5,decay=0, nesterov=False)#sgd = keras.optimizers.RMSprop(lr=0.01, rho=0.9, epsilon=None, decay=0.0000) 
    ...
    history = model.fit(inputimg2, outputimages2,
              batch_size=1,
              epochs=1000,
              verbose=0, callbacks=[wh0,cov_dump,rv])
...
def test_mnist():     
    ...
    from keras import backend as K
    from keras_utils import RecordVariable, PrintVariableStats, SGDwithLR
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    config.gpu_options.per_process_gpu_memory_fraction = 0.1
   # Create a session with the above options specified.
...
#    
    ...
    if test_acnn:
        fw = 7
        model.add(Conv2DAdaptive(rank=2,nfilters=24,kernel_size=(fw,fw), 
        ...
        fw = 7
        model.add(Conv2D(24, (fw, fw), activation='linear', 
    ...
    from lr_multiplier import LearningRateMultiplier
    lr=0.001
    #multipliers = {'acnn-1/Sigma:0': 1.0,'acnn-1/Weights:0': 1.0,
    #               'acnn-2/Sigma:0': 1.0,'acnn-2/Weights:0': 1.0}
    #opt = LearningRateMultiplier(SGD, lr_multipliers=multipliers,
...
#                             save_best_only=True)
    ...
    if test_acnn:
        pr_1 = PrintVariableStats("acnn-1","Weights:0",stat_func_list,stat_func_name)
        pr_2 = PrintVariableStats("acnn-1","Sigma:0",stat_func_list,stat_func_name)
        ...
        pr_1 = PrintVariableStats("conv2d_3","kernel:0",stat_func_list,stat_func_name)
        rv_weights_1 = RecordVariable("conv2d_3","kernel:0")
        callbacks+=[pr_1, rv_weights_1]
    pr_3 = PrintVariableStats("conv2d_1","kernel:0",stat_func_list,stat_func_name)
    ...
    if plt and test_acnn:
        ...
        for i in range(num_show):
            ...
            #print("U -shape: ", acnn_layer.U().shape,type(K.eval(acnn_layer.U()[:,:,0,i])))
            #print("Prod-shape", (ws[1][:,:,0,i]*acnn_layer.U()[:,:,0,i]).shape)
            plt.imshow(np.float32(wcnn[0][:,:,0,indices[i]]))
