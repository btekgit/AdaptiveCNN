...
from keras_utils import SGDwithLR, SGDwithCyclicLR
...
def resnet_layer(inputs,
    ...
    if not acnn:
        ...
        conv = Conv2DAdaptive(rank=2,nfilters=num_filters,
...
def resnet_v1(input_shape, depth, num_classes=10):
    ...
    x = resnet_layer(inputs=inputs,num_filters=num_filters,kernel_size=5)
    # Instantiate the stack of residual units
    for stack in range(2):
        for res_block in range(num_res_blocks):
            strides = 1
            if stack > 0 and res_block == 0:  # first layer but not first stack
                strides = 2  # downsampleConsole 24
...
def resnet_v2(input_shape, depth, num_classes=10):
    ...
    # Arguments
        input_shape (tensor): shape of input image tensor
        depth (int): number of core convolutional lregularizers.l2(weight_decay),ayers
        num_classes (int): number of classes (CIFAR10 has 10)
...
if version == 2:
    ...
    inputs = Input(shape=input_shape)
    outputs= resnet(inputs,num_classes,num_blocks=3, kernel_size=akernel_size[0],
                   acnn=acnn)
...
lr_dict = {'all':0.001, 'Sigma':0.001}
for i in lr_dict.keys(): 
    lr_dict[i]*=1.0#settings['lr_multiplier']
...
opt = SGDwithCyclicLR(peaklriter=epochs/2*e_i,lr=lr_dict, momentum = mom_dict, 
                      min_lr ={'all':0.0001}, peak_lr={'all':0.5,'Sigma':0.1},
                      dropsigma=0.5,
                      clips=clip_dict,clipvalue=1.0,verbose=2) 
# gives 89.24 at 150 epochs


pwl = lambda t: np.interp(t,[0, 15, 30, 35], [0, 0.1, 0.005, 0])
from keras.optimizers import SGD, Nadam
#opt = SGDwithLR(lr=0.001,momentum=0.9,nesterov=True, decay = 5e-4*128)
#opt = Nadam()
...
if acnn:
    model_name = 'cifar10_%s_acnn_resnet_model_best_sofar.h5' % model_type
else:
    model_name = 'cifar10_%s_resnet_model_best_sofar.h5' % model_type
...
print('Max val:',np.max(his.history['val_acc'])," at epoch: ", np.argmax(his.history['val_acc'])+1)
