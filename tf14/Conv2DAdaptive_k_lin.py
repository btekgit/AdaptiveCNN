...
#                                      constraint=None)
    ...
    def call(self, inputs):
        ...
        self.kernel=None
        u = self.U()
        kernel = samplefocus3(u)
        
        #print(K.eval(u))
        #print(K.eval(kernel))
        print("----------KERNEL-------------------------------------")
        print(K.eval(kernel)[:,:,0,0])
        print(K.eval(kernel)[:,:,0,1])
        print("-----------------------------------------------")
    ...
    def weight_initializer(self,shape, dtype='float32'):
        ...
        kernel = K.eval(samplefocus3(self.U()))
...
def test():
   ...
   # Create a session with the above options specified.
    ...
    node_acnn = Conv2DAdaptive(rank=2,nfilters=3,kernel_size=(5,5), 
                             ...
                             init_sigma=2.0, trainSigmas=True, 
                             trainWeights=True,norm=0)(node_in)
   ...
   # model.summary()
    ...
    opt = SGDwithLR(lr=lr_dict, momentum = mom_dict, 
                    clips=clip_dict, clipvalue=0.01)
    #(lr_dict, mom_dict) 
    #
    ...
    history = model.fit(inputimg2, outputimages2,
              batch_size=1,
              epochs=1000,
              verbose=2, callbacks=[wh0,cov_dump,rv])
