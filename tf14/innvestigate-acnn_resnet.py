...
from resnet_acnn_test_trials import test
...
#settings
settings = {'exname':'resnet', 'dset':'cifar10', 'depth':3, 
              'test_layer':'conv',
              'epochs':150, 'batch':128, 
...
# In[ ]:
epbckup = settings['epochs'] 
settings['epochs']=0
sc, hs, model = test(settings,sid=31+19*17)

fname = 'outputs/resnet_'+settings['dset']+'_'+settings['test_layer']+'_'+str(
        settings['kernel_size'])+'_'+str(settings['nfilters'])+'_Epc'+str(
                settings['epochs'])+''
         
settings['epochs'] = epbckup
load_models = True
if load_models:
    try:
        model.load(fname)
    except:
        print(fname+":can not be loaded")
        sc, hs, model = test(settings,sid=31+19*17)
...
for method in methods:
    analyzer = innvestigate.create_analyzer(method[0],        # analysis method identifier
                                            ...
                                            **method[1],allow_lambda_layers=True)      # optional analysis parameters
...
if settings['dset']=='mnist':
    ...
    selected_examples=[2386, 9269, 5886, 9435, 9616, 2694, 4841, 9906, 4992, 1182]
    
test_images = list(zip(x_test[selected_examples], np.argmax(y_test[selected_examples],axis=1)))

analysis = np.zeros([len(test_images), len(analyzers)]+list(x_train[0].shape))
text = []
...
# concat both
    def repmat_norm(imx, a):
        ...
        imx_pos = imx*(imx>=0.001).astype('int')
        mn = np.min(imx_pos)
        mx = np.max(imx_pos)
        imx_pos = (imx_pos-mn)/(mx-mn)
        #imx_pos[imx_pos==0]=0.5
        
        imx_neg = np.abs(imx*(imx<-0.001).astype('int'))
        mn = np.min(imx_neg)
        mx = np.max(imx_neg)
        ...
        if imx.shape[-1]==1:
            ...
            b = 0.8+0.2*(a-np.min(a))/(np.max(a)-np.min(a))
        imx3 = b
        print(imx3.shape, imx_pos.shape)
        if imx3.shape[-1]==1:
