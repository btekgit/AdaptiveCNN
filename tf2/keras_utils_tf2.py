# -*- coding: utf-8 -*-
"""
Created on Fri Jul 24 19:28:17 2020
...
class RenormCallback(Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        for i,p in enumerate(all_weights):
            ...
            if (p.name.find(self.varname)>=0):
                pval = p.numpy()
                pn1_sum_e = np.sum(pval**2,axis=(0,1,2))
                pshape = pval.shape[:-1]
                #sum_axis_int = np.arange(0,len(pval.shape)-1)
                #print(pshape,np.sqrt(np.prod(pshape)))
                pn1 = pval/np.sqrt(np.sum(pval**2, axis=(0,1,2), keepdims=True))*np.sqrt(2.0)
                pn1_sum_p = np.sum(pn1**2,axis=(0,1,2))
                p.assign(pn1)      
                tf.print("\n Renorming ", p.name, "shape", pshape, "from sum:", 
                         pn1_sum_e[0:3],  " to sum: ", pn1_sum_p[0:3], output_stream=sys.stdout)
